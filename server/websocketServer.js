...
async function handleJoin(ws, msg, client, room) {
  if (isViewer(msg)) {
    ...
    send(ws, { type: 'JOIN_OK', id: '', nickname: 'viewer', skin: 0, cat: 0, viewer: true });
    send(ws, { type: 'PLAYER_LIST', players: room.playersForClient(), world: room.worldForClient() });
    return;
  }

  if (room.isFull() && !client.id) {
    send(ws, { type: 'ERROR', msg: `Sala llena. Máximo ${MAX_PLAYERS} jugadores.` });
    return;
  }
...
function handleMessage(ws, raw, room) {
  ...
  // Mensaje compatible con clientes simples: { type:'MOVE', dir:'LEFT'|'RIGHT'|'JUMP'|'STOP' }
  if (msg.type === 'MOVE') {
...
function startHttpServer() {
  ...
  app.get('/health', (req, res) => {
    res.json({ ok: true, game: 'Ocean Park', server: 'websocket', playersMax: MAX_PLAYERS });
  });

  app.get('/web', (req, res) => {
    const apkUrl = SERVER_HOST.startsWith('localhost')
      ? `http://${SERVER_HOST}/apk`
      : `https://${SERVER_HOST}/apk`;
  ...
  app.get('/apk', (req, res) => {
    ...
    res.download(apkPath, 'ocean-park.apk');
  });
...
async function main() {
  ...
  const room = new GameRoom({
    ...
    mongo: {
      ...
      markKeyObtained: id => mongo.markKeyObtained(id, log),
